...
" Enable typing normal mode commands on a Russian keyboard
...
let s:autoload = stdpath('data') . '/site/autoload/plug.vim'
let s:plugdir  = stdpath('data') . '/plugged'

if empty(glob(s:autoload))
  " Use execute so we can build the command string safely
  silent execute '!curl -fLo ' . shellescape(s:autoload)
        \ . ' --create-dirs https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim'
  " Make it available right now (first run)
  execute 'source ' . fnameescape(s:autoload)
endif
...
" `:GV` for commit browser, `:GV!` for curr file only, `:GV?` for version list
